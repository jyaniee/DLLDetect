...
Windows 환경에서 DLL 인젝션 행위를 탐지·분석하고 결과를 직관적으로 시각화하는 보안 도구입니다.
...
## 프로젝트 개요
DLLDetect는 Windows 프로세스에 로드된 DLL을 대상으로
1. 정적 탐지(Static Detection) — 해시 검증, 코드 서명, 화이트리스트·ML 필터링
2. 동적 탐지(Dynamic Detection) — 디버그 이벤트 기반 인젝션 행위 감시  
...
| 정적 탐지 | **해시 기반 무결성 검증** | SHA-256 해시 비교로 파일 변조 및 비인가 DLL 식별 |
| 정적 탐지 | **코드 서명 검증** | `WinVerifyTrust` API를 통해 서명 유효성 확인 |
| 동적 탐지 | **디버그 이벤트 모니터링** | `CreateRemoteThread` / `LoadLibrary` 이벤트를 감시하여 인젝션 시도 탐지 |
| 로깅 | **CSV 로그 관리** | 정적 탐지 결과를 데스크톱 경로에 CSV로 저장 |
...
본 프로젝트는 **보안 연구 및 교육 목적**으로 개발되었습니다.  
악의적 사용 또는 무단 상업적 이용을 금지합니다.
